		...
		foreach($lines as $line) {
			if (strlen($line) && (substr($line, 0, 1) != '#') && $buttonCount < 12) {
				...
				if ($index !== false) {
					...
					$buttonString .= '<input id="' . $buttonName . '" type="button" value="' . $buttonName . '" onclick="send_cmd(' . "'bash " . $macroName . "'" . ')" class="' . $className . '" ' . $otherAtt . '>' . "\r\n";
					$buttonCount += 1;
				}
			}
		}
...
<html>
   ...
   <body onload="setTimeout('init(<?php echo "$mjpegmode, $video_fps, $divider" ?>);', 100);">
      ...
      <div id="secondary-buttons" class="container-fluid text-center">
         ...
         <a href="program.php" class="btn btn-default" <?php getdisplayStyle('preview', $userLevel); ?>>Program Mission</a>
         &nbsp;&nbsp;
